...
const findLiveSourceSignature = (
  ...
  if (
    !state ||
    !Array.isArray(state.sourceLines) ||
...
const getOrCreateScene = (options, attributes = {}) => {
  const runtime =
    resolveRuntime(options && options.runtime ? options.runtime : null) ||
...
const sceneMixin = {
  ...
  _add(geometry, material, options) {
    ...
    if (geometry instanceof THREE.Object3D) {
      ...
      if (
        geometry instanceof GlslSource ||
        (material && material.type === "quad")
      ) {
        options = material;
        material = geometry;
        geometry = null;
      ...
      switch (type) {
        case "points":
          object = getOrCreatePoints(
            Object.assign({ geometry, material }, options),
            this._runtime,
          );
          break;
        case "line loop":
  ...
  _createMesh(geometry, material, options = {}) {
    ...
    if (options.type === "quad") {
      ...
      mesh = getOrCreateInstancedMesh(
        Object.assign(
          { geometry, material, count: options.instanced },
          options,
        ),
        this._runtime,
      );
  ...
  _mesh(geometry, material, options) {
    const { internal, options: normalizedOptions } =
      normalizeInternalCall(options);
    if (!internal) {
...
class TriodeScene extends THREE.Scene {
  ...
  getCamera(options) {
    return (
      this._camera || ((options && options.out) || this.defaultOutput)._camera
    );
  }
