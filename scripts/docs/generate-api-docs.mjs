...
- \`TriodeRenderer\` members: **${manifest.triodeRendererMembers.length}**
- \`TriodeSynthApi\` members: **${manifest.triodeSynthMembers.length}**
- \`TriodeSceneApi\` members: **${manifest.sceneMembers.length}**
...
${toSignatureList(manifest.triodeRendererEntries, rendererNotes)}

## \`TriodeSynthApi\` Members

${toSignatureList(manifest.triodeSynthEntries, synthNotes)}
...
const main = async () => {
  ...
  const constructorOptionEntries = extractTopLevelEntries(
    extractBraceBlock(typesSource, "export interface TriodeOptions {"),
  );
  const triodeRendererEntries = extractTopLevelEntries(
    extractBraceBlock(typesSource, "declare class TriodeRenderer {"),
  );
  const triodeSynthEntries = extractTopLevelEntries(
    extractBraceBlock(typesSource, "export interface TriodeSynthApi {"),
  );
  const sceneEntries = extractTopLevelEntries(
    extractBraceBlock(typesSource, "export interface TriodeSceneApi {"),
  );
  const transformChainEntries = extractTopLevelEntries(
    extractBraceBlock(typesSource, "export interface TriodeTransformChain {"),
  );
  ...
  const apiManifest = {
    ...
    triodeRendererMembers: toUniqueList(
      triodeRendererEntries.map((entry) => entry.name),
    ),
    triodeSynthMembers: toUniqueList(
      triodeSynthEntries.map((entry) => entry.name),
    ),
    ...
    counts: {
      ...
      triodeRendererMembers: triodeRendererEntries.length,
      triodeSynthMembers: triodeSynthEntries.length,
