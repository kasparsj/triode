# triode Documentation

triode keeps a Hydra-style live-coding workflow while extending it with a Three.js scene API.

## Start here

- New to triode: [Getting Started](./getting-started/index.md)
- Fast first result: [Quickstart](./getting-started/quickstart.md)
- Mental model shift from Hydra: [Core Concepts](./concepts/index.md)

## Core guides

- [API Reference Overview](./api/index.md)
- [Generated API Reference](./api.md)
- [Cookbook](./recipes/index.md)
- [Livecoding Workflow](./workflows/livecoding.md)
- [Hydra Interop](./interop/hydra-in-triode.md)

## Operations and reliability

- [Troubleshooting + FAQ](./support/troubleshooting-faq.md)
- [Reference Index](./reference/index.md)
- [Release Notes](./reference/release-notes.md)
- [Documentation Checklist](./reference/documentation-checklist.md)

## What changed in this docs pass

- Added a Hydra-to-Triode translation layer for common workflows.
- Added task-oriented indexes for API and examples.
- Added navigation improvements: grouped sidebar, search, and next/previous links.
