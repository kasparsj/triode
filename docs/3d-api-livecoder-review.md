...
Update (2026-02-23): compatibility deprecation warnings now fire once for ambiguous `rotate(...)` usage and direct underscore scene helper calls (for example `_mesh(...)`), with constructor `legacy: true` providing an opt-in compatibility mode that restores legacy defaults (`makeGlobal: true`, `liveMode: "restart"`) and suppresses those warnings. Implementation and coverage are in `src/generator-factory.js`, `src/three/scene.js`, `src/hydra-synth.js`, `src/index.d.ts`, `docs/reference/parameter-reference.md`, and `scripts/smoke/browser-non-global-smoke.mjs`.
